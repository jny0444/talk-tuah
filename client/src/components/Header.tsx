    ...
    return (
      <div className="flex flex-row justify-between items-center px-16 py-8">
        <div className='flex flex-row gap-6 items-center'>
          ...
          <h2 className='font-lexend text-3xl font-semibold select-none'>Tawk Twoah</h2>
        </div>
        <button className="group/button relative inline-flex items-center justify-center overflow-hidden rounded-md bg-black backdrop-blur-lg px-6 py-2 text-base text-white transition-all duration-300 ease-in-out hover:scale-105 active:scale-100 hover:shadow-xl hover:shadow-gray-600/50 border border-white/20">
          <span className="text-lg font-lexend">Connect Wallet<span className='[word-spacing:0.2rem]'> <FontAwesomeIcon icon={faWallet} /></span></span>
          <div className="absolute inset-0 flex h-full w-full justify-center [transform:skew(-13deg)_translateX(-100%)] group-hover/button:duration-1000 group-hover/button:[transform:skew(-13deg)_translateX(100%)]">
            <div className="relative h-full w-10 bg-white/20"></div>
          </div>
        </button>
